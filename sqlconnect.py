...
def time(id_num,specialty):
	...
	conn.close() 
	return t
def book_appointment(id_num,specialty,mail_id):
	conn = pymysql.connect(
		host='bpfsakhgvayk9oawkrtl-mysql.services.clever-cloud.com',
        user='uhc4dju2xkqp2shl',
        password='bXundo1ws1qmMyu1Yyhu',
	...
	table=specialty
	cur = conn.cursor()
	sql = "UPDATE %s " %table + " SET Status = 'booked' , info=%s WHERE id = %s"
	cur.execute(sql,(mail_id,id_num))
...
def cancel_appointment(mail_id,specialty):
	conn = pymysql.connect(
		host='bpfsakhgvayk9oawkrtl-mysql.services.clever-cloud.com',
        user='uhc4dju2xkqp2shl',
        password='bXundo1ws1qmMyu1Yyhu',
	...
	cur = conn.cursor()
	sql = "UPDATE %s " %specialty + " SET status = 'empty' WHERE info = %s "
